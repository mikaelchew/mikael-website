...
- Rewrite the Mailchimp `book-chapter` automation to send the **link** to `/chapter-1.html` rather than attaching the PDF — subject, preview text and bilingual body are drafted in `MAILCHIMP_CHAPTER_EMAIL.md`, together with the steps. That automation lives in Mailchimp's cloud, not in this repo, so only Mikael can change it.
